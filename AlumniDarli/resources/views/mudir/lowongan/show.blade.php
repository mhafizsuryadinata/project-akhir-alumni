...
@section('content')
<div class="mb-4">
    <a href="{{ route('mudir.lowongan.index') }}" class="btn btn-outline-secondary rounded-pill px-4">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="pe-3 text-end">
                                    <div class="btn-group">
                                        @if($lamaran->cv_path)
                                            <a href="{{ route('mudir.lowongan.cv', $lamaran->id) }}" class="btn btn-sm btn-outline-primary" title="Download CV">
                                                <i class="fas fa-download"></i>
                                            </a>
                                        @endif
                                        
                                        @if($lamaran->status_pimpinan == 'Menunggu')
                                            <form action="{{ route('mudir.lowongan.status', $lamaran->id) }}" method="POST" class="d-inline">
                                                @csrf
                                                @method('PATCH')
                                                <input type="hidden" name="status" value="Diterima">
                                                <button type="submit" class="btn btn-sm btn-outline-success" title="Terima (Pimpinan)" onclick="return confirm('Terima lamaran ini sebagai Pimpinan?')">
                                            ...
                                            <form action="{{ route('mudir.lowongan.status', $lamaran->id) }}" method="POST" class="d-inline">
                                                ...
                                                <button type="submit" class="btn btn-sm btn-outline-danger" title="Tolak (Pimpinan)" onclick="return confirm('Tolak lamaran ini sebagai Pimpinan?')">
                                        ...
                                        @else
                                            <form action="{{ route('mudir.lowongan.status', $lamaran->id) }}" method="POST" class="d-inline">
                                                ...
                                                <button type="submit" class="btn btn-sm btn-outline-secondary" title="Reset ke Menunggu" onclick="return confirm('Reset status moderasi Pimpinan?')">
